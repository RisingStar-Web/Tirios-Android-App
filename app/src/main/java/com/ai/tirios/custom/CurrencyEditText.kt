...
class CurrencyEditText : AppCompatEditText {
    ...
    private var current = ""
    private val editText: CurrencyEditText = this@CurrencyEditText
    ...
    */
    ...
    fun getCleanStringValue(): String {
        var value = "0.00"
        if (Decimals && !TextUtils.isEmpty(editText.text.toString())) {
            value = String.format("%.2f", editText.text.toString().trim()
                .replace("[$,]".toRegex(), "").replace(Currency.toRegex(), "").toDouble())
        } else if(!TextUtils.isEmpty(editText.text.toString())){
            val cleanString = editText.text.toString().trim { it <= ' ' }
                .replace("[$,.]".toRegex(), "").replace(Currency.toRegex(), "")
                .replace("\\s+".toRegex(), "")
            try {
                value = String.format("%.2f", cleanString.toDouble())
            } catch (e: NumberFormatException) {
            }
        }
        return value
